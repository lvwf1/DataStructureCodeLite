...
// TEMPLATE CLASS PROVIDED: graph<Item> (a class for labeled weighted graphs)
...
//
//   void add_edge(size_t source, size_t target, std::size_t weight)
//     Precondition: (source < size( )) and (target < size( ) and weight > 0).
...
//
//   std::size_t weight (std::size_t source, std::size_t target)
//     Precondition: source < size()  and target < size()
//     Postcondition: The return value is the weight of the edge from
//      source to target.  0 is returned if there is no edge
//
...
namespace main_savitch_15
{
    ...
    public:
        ...
        void add_vertex(const Item& label);
        void add_edge(std::size_t source, std::size_t target, std::size_t weight);
        ...
        std::size_t weight (std::size_t source, std::size_t target) const;
    private:
        std::size_t edges[MAXIMUM][MAXIMUM];
        Item labels[MAXIMUM];
